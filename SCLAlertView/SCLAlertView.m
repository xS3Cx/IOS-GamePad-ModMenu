...
@property (strong, nonatomic) UIView *lineAboveGradientView;
@property (strong, nonatomic) UIView *lineBelowGradientView;
...
- (void)setupViewWindowWidth:(CGFloat)windowWidth
{
    ...
    _lineAboveGradientView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, windowWidth, lineHeight)];
    _lineBelowGradientView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, windowWidth, lineHeight)];
    
    [_contentView addSubview:_lineAboveGradientView];
    [_contentView addSubview:_lineBelowGradientView];
    
    [Colors initializeColors];
    
    // Tworzenie warstwy gradientu dla górnej linii
    CAGradientLayer *topLineGradientLayer = [CAGradientLayer layer];
    topLineGradientLayer.frame = _lineAboveGradientView.bounds;
    ...
    [_lineAboveGradientView.layer addSublayer:topLineGradientLayer];
    
    // Tworzenie warstwy gradientu dla dolnej linii
    CAGradientLayer *bottomLineGradientLayer = [CAGradientLayer layer];
    bottomLineGradientLayer.frame = _lineBelowGradientView.bounds;
    ...
    [_lineBelowGradientView.layer addSublayer:bottomLineGradientLayer];
    
    // Dodajemy UIImageView
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake((windowWidth - 60) / 2, -30.3333, 60, 55)]; // Centered horizontally
    [self.view addSubview:imageView];
    
    // Base64 zakodowany obraz
    NSString *base64String = [Heaven getMenuButton];
...
- (void)viewWillLayoutSubviews
{
    ...
    // Update gradient lines positions and corner radius
    _lineAboveGradientView.frame = CGRectMake(0, 0, _windowWidth, 2.0);
    _lineBelowGradientView.frame = CGRectMake(0, _windowHeight - 2.0, _windowWidth, 2.0);
    
    // Set corner radius for top line (top corners only)
    UIBezierPath *topMaskPath = [UIBezierPath bezierPathWithRoundedRect:_lineAboveGradientView.bounds 
                                                     byRoundingCorners:(UIRectCornerTopLeft | UIRectCornerTopRight)
                                                           cornerRadii:CGSizeMake(10.0, 10.0)];
    CAShapeLayer *topMaskLayer = [CAShapeLayer layer];
    topMaskLayer.path = topMaskPath.CGPath;
    _lineAboveGradientView.layer.mask = topMaskLayer;
    
    // Set corner radius for bottom line (bottom corners only)
    UIBezierPath *bottomMaskPath = [UIBezierPath bezierPathWithRoundedRect:_lineBelowGradientView.bounds 
                                                      byRoundingCorners:(UIRectCornerBottomLeft | UIRectCornerBottomRight)
                                                            cornerRadii:CGSizeMake(10.0, 10.0)];
    CAShapeLayer *bottomMaskLayer = [CAShapeLayer layer];
    bottomMaskLayer.path = bottomMaskPath.CGPath;
    _lineBelowGradientView.layer.mask = bottomMaskLayer;
    
    // Update gradient layers frames
    for (CALayer *layer in _lineAboveGradientView.layer.sublayers) {
        if ([layer isKindOfClass:[CAGradientLayer class]]) {
            layer.frame = _lineAboveGradientView.bounds;
        }
    }
    
    for (CALayer *layer in _lineBelowGradientView.layer.sublayers) {
        if ([layer isKindOfClass:[CAGradientLayer class]]) {
            layer.frame = _lineBelowGradientView.bounds;
        }
    }
    
    // Set alert corner radius to 0 for both content view and background
    ...
    for (SCLButton *btn in _buttons) {
        ...
        // Update gradient layer for the button
    ...
    _contentView.layer.cornerRadius = self.cornerRadius ? self.cornerRadius : 10.0f;
}
...
- (SCLButton *)addButton:(NSString *)title {
    ...
    // Ensure button background is clear
